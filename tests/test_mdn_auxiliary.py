...
def test_auxiliary_model_single_and_batch_output_shapes():
    model = MotiveDecompositionNetwork(input_dim=8, num_skills=16, num_objectives=2)

    single_context = torch.randn(8)
    single_skill_id = torch.tensor(3)
    batch_context = torch.randn(4, 8)
...
def test_auxiliary_model_checkpoint_round_trip():
    ...
    try:
        ...
        restored = MotiveDecompositionNetwork(input_dim=8, num_skills=16, num_objectives=2)
